...
<div class="flex">
    <div class={`fixed top-0 left-0 h-full bg-olive-500 text-white w-64 transform transition-transform duration-300 
        ...
        <div class="h-20 flex flex-row items-center justify-between">
            ...
            {#if $isSidebarOpen}
                <button class="mr-4 h-8 w-8 text-white rounded-sm rounded-xl shadow-sm hover:bg-taupe-300 transform flex flex-row items-center justify-center" onclick={toggleSidebar}>
        ...
        <ul class="space-y-2 px-4">
            ...
            { #if !$jwt }
                <li class="hover:bg-zinc-500 rounded-sm border text-indigo-200 border-indigo-200">
